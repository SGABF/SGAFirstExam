	...
	//한글자씩 지우기
		...
		$("#remove").click(function(){
			...
			if(value!=null && value.length>0){
				...
				// 그 값으로 내용을 변경한다.
				$("#heightbar").val(value);
				}else if(document.getElementById("weightbar")){
...
<body>
	<form action="${pageContext.request.contextPath}/Calc/BmiResult.jsp"
		...
		<table class = "bg-success p-2 text-dark bg-opacity-10">
			<tr>
				<td colspan="4" style="text-align: center; font-size:25px;">BMI 계산기</td>
			</tr>
			<tr>
			...
			<tr>
				<td colspan="3" style="text-align: center">신장 : <input
					type="text" id="heightbar" name="heightbar" readonly="readonly" style="width: 300px; text-align: center;"/>
				</td>
			</tr>
			<tr>
				<td colspan="3" style="text-align: center">체중 : <input
					type="text" id="weightbar" name="weightbar" readonly="readonly" style="width: 300px; text-align: center;"/>
				</td>
			</tr>
			<tr>
					
				<td><input type="button" value="전체 삭제" id="c" name="c"
					onClick="Clear()" class="btn btn-outline-dark"
					/></td>
			...
			<tr>
				...
				<td><input type="button" value="9" id="9" name="9"
					class="btn btn-success btn-lg"
					onClick="ClickNum(this.id)" /></td>
				<td><input type="button" value="←" id="remove" name="remove" class="btn btn-outline-dark"></td>
			</tr>
			<tr>
			...
			<tr>
				<td><input type="button" value="0" id="0" name="0"
					class="btn btn-success btn-lg"
					onClick="ClickNum(this.id)" 
					/></td>
			</tr>
			<tr>
				<td><input type="button" value="결과 확인" id="result" name="result" onclick="getBMI()" class="btn btn-outline-dark" style="height: 48px;"/></td>
